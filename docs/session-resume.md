...
- PR #378 merged the Snowflake transform objective as protected main `bb06aa8`; exact-main run
  ...
  One exact-RC29 candidate completed the 100,000-fact/100-dimension transform shape in 98.301
  seconds with zero retries, passed scan, join, aggregation, the two-row incremental merge, 21
  assertions, exact readback, and cleanup, and left zero staging or named provider resources by
  125.769 seconds. Provider metering has not posted, so cost and normalized status remain
  `not_evaluated` under the held USD 0.50 bound, leaving USD 1.75 unreserved. This closes functional
  Snowflake transform only; failure behavior follows separately from fresh protected main. See
...
- PR #369 merged the Snowflake bulk objective as protected main `26bcedd`; exact-main run
  ...
  500,000 narrow and 200,000 wide rows in 93.736 seconds with zero retries; both COPY completion
  and throughput objectives passed. Interactive OAuth timeouts before launch consumed no candidate
  attempt and were cleaned inside their rails. Final cleanup left zero named Snowflake objects or
  candidate containers within a 4.49-minute upper-bound lifetime. Provider-measured cost is not yet
  available in retained evidence, so cost and normalized status remain `not_evaluated` under the
  held USD 0.50 bound, leaving USD
  3.25 unreserved. This closes functional Snowflake bulk only, not its cost gate or other Phase 8
  classes. See
...
- DANDER-208 and DANDER-209 are included in private `0.9.0rc28` from exact protected main
  `7135b8c`, whose exact-main run `31961210116` passed all five jobs before publication. Its
  source-free amd64/arm64 GAR index is `sha256:f8259276…f94959e`; exact-wheel, rootless read-only,
  stable qualification-entrypoint, GCP/Kubernetes/external-AWS/external-Azure selector, SPDX SBOM,
  and SLSA provenance checks passed. This is private candidate evidence only. No Azure mutation may
