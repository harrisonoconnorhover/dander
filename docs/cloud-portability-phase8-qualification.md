...
`32037495657` passed all five jobs before mutation. Three interactive OAuth callbacks expired
before candidate launch; each was classified as operator preflight, automatic retry stayed off,
and the only preflight that had created a database and initially suspended warehouse cleaned them
up in 19.52 minutes without reaching Python or starting warehouse compute. The final fresh OAuth
session then launched the one permitted exact-RC29 candidate through `dander qualification-run`.
It processed 500,000 narrow and 200,000 wide rows, 237,600,000 logical bytes total, in 93.736
seconds with 303,357,952 bytes peak RSS and zero retries. Narrow and wide COPY completion and
throughput objectives passed at 13,829.346 and 5,054.206 rows/second. Cleanup left zero staging
tables, stages, named databases, warehouses, roles, or candidate containers within a 4.49-minute
upper-bound final resource lifetime. Provider-measured cost is not yet available in retained
evidence, so the normalized report and cost objective remain `not_evaluated`; the full USD 0.50
bound stays held, leaving USD
3.25 unreserved. This closes the functional Snowflake bulk execution only, not provider cost,
other scale classes, pairwise, soak, final closure, release, or support. See
...
`catalog=postgresql` although no catalog operation ran. Provider billing later posted for the exact
project and August 16 charge day: Compute Engine net USD 0.05, Kubernetes Engine net USD 0.00 after
credits, and Networking net USD 0.00 after credits. The USD 0.05 total passes the approved USD 0.50
ceiling. A derived final report corrects only the unused catalog metadata to `none`; the raw report
is unchanged. All five bounded-memory objectives now pass. See
...
`32046930482` passed all five jobs before mutation. The live trial display showed `$398 of $400`
left before execution and after cleanup. One exact-RC29 candidate seeded 300,000 rows, then applied
1,500 updates and 1,500 inserts in a 3,000-row delta. It completed in 49.596 seconds with zero
retries, 11 COPY operations, 285,429,760 bytes peak RSS, exact 301,500-row readback, and zero
cursor-regression rows. All five non-cost objectives passed. Cleanup removed the database,
warehouse, role, staging objects, and candidate container within a 5.19-minute resource-lifetime
upper bound. Provider metering has not posted, so the normalized report and cost objective remain
`not_evaluated`; the full USD 0.50 bound stays held, leaving USD 2.75 unreserved. This closes only
functional Snowflake incremental execution, not provider cost, remaining scale classes, pairwise,
...
five jobs before mutation. Two bounded interactive OAuth preflights never reached the candidate and
consumed no candidate attempt. The first created only the minimum runtime role, then removed it and
verified all approved names absent inside 47.812 seconds; the second timed out before any owned
resource existed. One exact-RC29 candidate then scanned 100,000 facts, joined 100 dimensions,
aggregated ten categories, applied the two-row incremental merge, and passed 21 generic and exact
assertions. It completed 100,001 result rows in 98.301 seconds at 1,017.294 rows/second with zero
retries, five COPY operations, four models, and zero staging residue. Cleanup started 123.425
seconds after the first role and verified zero database, warehouse, role, container, or provider
process by 125.769 seconds. All six non-cost objectives passed. Provider metering remains pending,
so the normalized report and cost objective stay `not_evaluated`; the full USD 0.50 bound remains
held, leaving USD 1.75 unreserved. This closes functional Snowflake transform only, not provider
...
Remaining work includes remaining benchmark classes/providers and Kubernetes soak;
