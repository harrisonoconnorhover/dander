...
Audited on 2026-08-09 against the product promise in `steering/00-project-overview.md`.
...
| Infrastructure reconciliation safety | Live-proven | Refreshed final plans reported `0` add, `0` change, and `0` destroy across `28` stage-zero and `113` platform resources. |

## Current release and deployment record

- Public Dander beta: `0.7.1`; public Salesforce connector: `0.3.1`; public ServiceNow connector:
  `0.2.2`.
...
- Accepted retained source-free image: `sha256:5f9db33…bb7d2`, built with Dander `0.7.1rc1`,
  Salesforce `0.3.1`, and ServiceNow `0.2.2`. Stable `0.7.1` has identical runtime source and will
  replace only the package-version image after publication.
- Retained Druff image: `sha256:a5e255d6…871c`; public static URL:
  <https://dander-druff-yos2b3gbca-uc.a.run.app>.
- Manual retained executions on 2026-08-09 completed successfully for Greenhouse, HubSpot,
  four-endpoint Salesforce, ServiceNow, and the executable graph. Four daily connector schedules
  are enabled; subsequent scheduled observation remains in the active operator soak.
- Greenhouse graph execution `dander-greenhouse-graph-djf4t` completed successfully on 2026-08-09;
...
- The Salesforce `0.3.1` plugin supports read-only Accounts, Contacts, Opportunities, and
