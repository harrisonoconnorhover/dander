...
class FailureDetails:
    """A stable code and non-sensitive message safe to persist."""
...
def classify_failure(error: Exception, *, stage: RunStage, run_id: str) -> FailureDetails:
    ...
    if stage.value == "transform":
        ...
        return _details(code, f"{action}. Inspect logs for run {run_id}.")
...
def _details(code: str, summary: str) -> FailureDetails:
    cleaned = " ".join(summary.split())[:_MAX_SUMMARY_LENGTH]
    return FailureDetails(code=code, summary=cleaned)
