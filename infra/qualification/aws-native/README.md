...
creator also provisions a `dander_runtime` database role with only the DDL, exact COPY-role, and
read-only `SYS_SERVERLESS_USAGE` permissions needed by the qualification writer and provider cost
report. The ASSUMEROLE grant names the same explicit COPY role ARN that the writer sends to Redshift;
the Fargate task maps `dander_runtime` through its `RedshiftDbRoles` IAM tag. Sensitive values remain
only in Secrets Manager and the existing encrypted remote state.
