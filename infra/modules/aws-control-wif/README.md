...
The GCP account can update/run/cancel Cloud Run Jobs, read their logs and BigQuery table metadata,
and act as only the runtime service accounts supplied by the existing scheduled-job module.
Pipeline workers continue to use their existing per-pipeline service accounts and BigQuery data
permissions.
